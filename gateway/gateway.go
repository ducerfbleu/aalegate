...
type config struct {
	...
	runID          string
	maxCapture     int64
	upstreamKey    string // key custody: real key injected upstream; the agent never sees it
	upstreamKeyHdr string // header to inject (default Authorization)
	upstreamKeyPfx string // value prefix (default "Bearer ")
}
...
func loadConfig() config {
	...
	// key custody: the gateway holds the real upstream key (env or mounted file) and injects it, so
	// the agent only ever sends a dummy. GATEWAY_UPSTREAM_KEY_FILE (mounted, not in `docker inspect`)
	// takes precedence over GATEWAY_UPSTREAM_KEY.
	upKey := os.Getenv("GATEWAY_UPSTREAM_KEY")
	if f := os.Getenv("GATEWAY_UPSTREAM_KEY_FILE"); f != "" {
		b, err := os.ReadFile(f)
		if err != nil {
			log.Fatalf("read GATEWAY_UPSTREAM_KEY_FILE %q: %v", f, err)
		}
		upKey = strings.TrimSpace(string(b))
	}
	return config{
		...
		maxCapture:     maxCap,
		upstreamKey:    upKey,
		upstreamKeyHdr: env("GATEWAY_UPSTREAM_KEY_HEADER", "Authorization"),
		upstreamKeyPfx: env("GATEWAY_UPSTREAM_KEY_PREFIX", "Bearer "),
	}
}
...
// dedup replaces rec.Request with a version containing only messages new since
// the previous chat.completions turn. Sets rec.MessagesOffset to the number of
// prior messages elided. Must be called between buildRecord and write.
func (c *chainLogger) dedup(rec *Record, rawBody []byte, maxCap int64) {
	// Both OpenAI chat.completions and Anthropic messages resend the full turn
	// history each call under a top-level "messages" array; elide the common prefix.
	if (rec.Endpoint != "chat.completions" && rec.Endpoint != "messages") || len(rawBody) == 0 {
		return
	}
	var rp struct {
		Messages []json.RawMessage `json:"messages"`
	}
	if json.Unmarshal(rawBody, &rp) != nil || len(rp.Messages) == 0 {
		return
	}

	c.mu.Lock()
	prev := c.prevMessages
	c.prevMessages = rp.Messages
	c.mu.Unlock()
	...
	prefixLen := 0
	limit := len(prev)
	if len(rp.Messages) < limit {
		limit = len(rp.Messages)
	}
	for i := 0; i < limit; i++ {
		if !bytes.Equal([]byte(prev[i]), []byte(rp.Messages[i])) {
			break
		}
		prefixLen++
	}
	if prefixLen == 0 || prefixLen >= len(rp.Messages) {
		return
	}

	var full map[string]json.RawMessage
	if json.Unmarshal(rawBody, &full) != nil {
		return
	}
	tail, err := json.Marshal(rp.Messages[prefixLen:])
	if err != nil {
		return
	}
	full["messages"] = tail
	dedupBody, err := json.Marshal(full)
	if err != nil {
		return
	}
	if int64(len(dedupBody)) <= maxCap {
		rec.Request = dedupBody
		rec.MessagesOffset = prefixLen
	}
}
...
func normEndpoint(path string) string {
	...
	case strings.HasSuffix(path, "/messages"):
		return "messages" // Anthropic Messages API (Claude Code)
...
// ---- parsing helpers (Anthropic Messages API shapes) -----------------------
//
// Claude Code speaks the Anthropic Messages API (POST /v1/messages), a different
// wire format from OpenAI: the request carries top-level `system` + `messages`;
// the response is a list of typed content blocks (text / thinking / tool_use)
// rather than choices/delta. These three fillers mirror fillRequest/fillNonStream/
// fillStream so a `messages` record ends up with the same normalized fields
// (completion, reasoning, tool_calls, usage, model, finish_reason).

// anthToolCall is the normalized tool-use shape logged for Anthropic turns. It
// deliberately differs from OpenAI's function-call shape (name/input vs
// function.name/function.arguments)---readers dispatch on the endpoint.
type anthToolCall struct {
	ID    string          `json:"id,omitempty"`
	Name  string          `json:"name"`
	Input json.RawMessage `json:"input"`
}

func (rec *Record) fillRequestAnthropic(body []byte, maxCap int64) {
	var rp struct {
		Model    string          `json:"model"`
		Stream   bool            `json:"stream"`
	...
	rec.Stream = rp.Stream
	switch {
	case len(rp.Messages) > 0: // hash the actual model input: system + messages
		rec.PromptSHA256 = sha(append(append([]byte{}, rp.System...), rp.Messages...))
	default:
		rec.PromptSHA256 = sha(body)
	}
	if int64(len(body)) <= maxCap {
...
func (rec *Record) fillNonStreamAnthropic(body []byte) {
	var rp struct {
		Model   string `json:"model"`
		Content []struct {
			Type     string          `json:"type"`
	...
	rec.FinishReason = rp.StopReason
	var content, reasoning strings.Builder
	var tools []anthToolCall
	for _, b := range rp.Content {
		switch b.Type {
		case "text":
		...
		case "tool_use":
			tools = append(tools, anthToolCall{ID: b.ID, Name: b.Name, Input: b.Input})
		}
	}
	rec.Completion = content.String()
	rec.Reasoning = reasoning.String()
...
// fillStreamAnthropic reconstructs a completion from an Anthropic SSE stream. Each
// `data:` line is a typed event (the `event:` lines are ignored---the `type` field
// inside the data payload is authoritative): message_start carries the model and
// input-token usage; content_block_start opens a text/thinking/tool_use block at an
// index; content_block_delta appends text_delta / thinking_delta / input_json_delta
// (tool arguments arrive as partial_json fragments, reassembled per index);
// message_delta carries stop_reason and the final output-token usage.
func (rec *Record) fillStreamAnthropic(body []byte) {
	type block struct {
		kind string // text | thinking | tool_use
		id   string
		name string
		json strings.Builder // tool_use input, reassembled from input_json_delta
	}
	blocks := map[int]*block{}
	var order []int
	var content, reasoning strings.Builder
	var model, finish string
	var inputUsage json.RawMessage
	...
	for sc.Scan() {
		...
		case "message_start":
			...
			if len(ev.Message.Usage) > 0 && string(ev.Message.Usage) != "null" {
		...
		case "message_delta":
			...
			if len(ev.Usage) > 0 && string(ev.Usage) != "null" {
				...
				if json.Unmarshal(ev.Usage, &u) == nil && u.Output != nil {
	...
	rec.FinishReason = finish
	rec.Usage = mergeAnthropicUsage(inputUsage, outputTokens)
	...
	for _, i := range order {
		...
		tools = append(tools, anthToolCall{ID: b.id, Name: b.name, Input: input})
	}
	if len(tools) > 0 {
		if b, err := json.Marshal(tools); err == nil {
			rec.ToolCalls = b
...
// mergeAnthropicUsage folds the streamed usage back into one object: the
// input-token counts arrive on message_start, the final output_tokens on
// message_delta. Returns the input usage with output_tokens overlaid.
func mergeAnthropicUsage(inputUsage json.RawMessage, outputTokens int64) json.RawMessage {
	m := map[string]json.RawMessage{}
	if len(inputUsage) > 0 && string(inputUsage) != "null" {
		if json.Unmarshal(inputUsage, &m) != nil {
			m = map[string]json.RawMessage{}
		}
	}
	if outputTokens >= 0 {
		m["output_tokens"] = json.RawMessage(strconv.FormatInt(outputTokens, 10))
	}
	if len(m) == 0 {
		return nil
	}
...
func buildRecord(m *reqMeta, cfg config, captured []byte, total int64, truncated bool) *Record {
	...
	anth := rec.Endpoint == "messages"
	parse := rec.Endpoint == "chat.completions" || rec.Endpoint == "completions" || anth
	if parse && len(m.reqBody) > 0 {
		if anth {
			rec.fillRequestAnthropic(m.reqBody, cfg.maxCapture)
		} else {
	...
	if parse && len(captured) > 0 && m.status >= 200 && m.status < 300 {
		switch {
		case anth && rec.Stream:
			rec.fillStreamAnthropic(captured)
		case anth:
			rec.fillNonStreamAnthropic(captured)
		case rec.Stream:
...
func main() {
	...
	proxy := &httputil.ReverseProxy{
		Rewrite: func(pr *httputil.ProxyRequest) {
			...
			// Drop the client's Accept-Encoding so our Transport requests gzip on its own and
			// TRANSPARENTLY DECODES the response before we tee it. Remote APIs (e.g. api.anthropic.com)
			// return gzip/br SSE; without this the codec scans compressed bytes and captures an empty
			// completion. The agent still receives a valid (identity) response from the proxy.
			pr.Out.Header.Del("Accept-Encoding")
			if cfg.upstreamKey != "" { // key custody: drop the agent's (dummy) auth, inject the real key
				pr.Out.Header.Del("Authorization")
				pr.Out.Header.Del("X-Api-Key")
				pr.Out.Header.Set(cfg.upstreamKeyHdr, cfg.upstreamKeyPfx+cfg.upstreamKey)
			}
		},
		Transport:     tr,
		...
		ModifyResponse: func(resp *http.Response) error {
			...
			resp.Body = &captureRC{
				...
				finalize: func(captured []byte, total int64, truncated bool, tsFirst, tsEnd time.Time) {
					...
					m.tsEnd = tsEnd
					rec := buildRecord(m, cfg, captured, total, truncated)
					cl.dedup(rec, m.reqBody, cfg.maxCapture)
					cl.write(rec)
				},
			}
			return nil

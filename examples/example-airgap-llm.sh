#!/usr/bin/env bash
# example-airgap-llm.sh --- prove aalegate airgap: only the LLM path gets through.
#
# Wires a tiny fake LLM behind the real aalegate gateway + recorder, runs a real
# aalegate-pi agent container on the airgap network, and proves:
...
#   ../build-podman.sh --from docker   # build aalegate-gateway + aalegate-pi
#   podman build -t aalegate-tiny-llm -f tiny-llm/Dockerfile tiny-llm
#
# Usage:
...
GW_NAME="aalegate-gw-test"
AGENT_NAME="aalegate-agent-test"
LLM_NAME="aalegate-tiny-llm"
LLM_IMAGE="aalegate-tiny-llm"
AGENT_IMAGE="aalegate-pi"
GW_IMAGE="aalegate-gateway"
AUDIT_ROOT="${HOME}/.local/state/aalegate"
...
echo "=== aalegate airgap + LLM path test ==="
echo "  audit: $AUDIT"
...
MODELS=$(podman exec "$AGENT_NAME" curl -sf --max-time 5 \
    -H "Authorization: Bearer aalegate" \
